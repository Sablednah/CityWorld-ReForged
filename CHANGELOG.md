...
- **A CityWorld End.** Customize has a new **Realms → End** choice (default Vanilla). *CityWorld islands*
  keeps vanilla's centre exactly as it is — the central island, the obsidian pillars, the exit portal and the
  dragon fight are generated by vanilla itself within 1,024 blocks of the middle — and fills the outer islands
  beyond the void with CityWorld cities in end stone, floating above nothing (fall off and you fall). Biomes
  follow the islands: tall ones highlands with chorus forests, lower ones midlands and barrens. **End cities
  still generate**, so elytra, shulkers and end rods are where they should be. Modpacks can lock it with
  `cityworldEnd = "cityworld"` in `config/cityworld-startup.toml`.
