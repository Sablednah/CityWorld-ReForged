...
/**
 * A {@link #Width}×{@link #Width} grid of chunks — the unit of city planning. Its constructor runs
 * the whole plan for that block of the world: the shape provider places roads, then a context fills
 * in the lots, then any remaining holes become nature.
 *
 * <p><b>Seed-deterministic, and it must stay that way.</b> Every decision here traces back to the
 * world seed and the platmap's own origin, never to how many platmaps have been built or in what
 * order. That is what lets the modern pipeline plan them concurrently, and what makes per-chunk
 * regeneration viable at P5. {@code CityWorldGenerator.getConnectionKey} had to be reworked to hold
 * this line — see its javadoc.
 */
