...
public abstract class AbstractBlocks {
	...
	protected abstract void setBlock(int x, int y, int z, Material material, SlabType type);
	...
	public final void setBlocks(int x, int y1, int y2, int z, Material material, SlabType type) {
		for (int y = y1; y < y2; y++)
			setBlock(x, y, z, material, type);
	}

	public final void setBlocks(int x1, int x2, int y, int z1, int z2, Material material, SlabType type) {
		for (int x = x1; x < x2; x++) {
			for (int z = z1; z < z2; z++) {
				setBlock(x, y, z, material, type);
			}
	...
	public final void setBlocks(int x1, int x2, int y1, int y2, int z1, int z2, Material material, SlabType type) {
		for (int x = x1; x < x2; x++) {
			for (int z = z1; z < z2; z++) {
				for (int y = y1; y < y2; y++)
					setBlock(x, y, z, material, type);
...
//	}
	...
	private void airoutBlock(CityWorldGenerator generator, int x, int y, int z, boolean forceIt) {
		if (forceIt || generator.clearAtmosphere())
			setAtmosphereBlock(x, y, z, generator.findAtmosphereMaterialAt(y));
	}
	...
	private void airoutBlocks(CityWorldGenerator generator, int x1, int x2, int y, int z1, int z2,
			boolean forceIt) {
		if (forceIt || generator.clearAtmosphere()) {
			Material air = generator.findAtmosphereMaterialAt(y);
			for (int x = x1; x < x2; x++)
				for (int z = z1; z < z2; z++)
