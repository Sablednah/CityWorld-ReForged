...
//					int lowestY = generator.streetLevel - cisternDepth + 1 + waterDepth;
	...
	private void generateTreePark(CityWorldGenerator generator, RealBlocks chunk, int surfaceY, int benchEnd,
		...
		if (benchStart == 3) {
			// cornet bit
			if (NW) {
				chunk.setStair(3, surfaceY, 3, stairs, BlockFace.NORTH, StairsShape.INNER_LEFT);
			}
			if (NE) {
				chunk.setStair(12, surfaceY, 3, stairs, BlockFace.NORTH, StairsShape.INNER_RIGHT);
			}
			if (SW) {
				chunk.setStair(3, surfaceY, 12, stairs, BlockFace.SOUTH, StairsShape.INNER_RIGHT);
			}
			if (SE) {
				chunk.setStair(12, surfaceY, 12, stairs, BlockFace.SOUTH, StairsShape.INNER_LEFT);
			}
		}
