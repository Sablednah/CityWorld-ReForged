...
public class UnfinishedBuildingLot extends BuildingLot {
	...
	public UnfinishedBuildingLot(PlatMap platmap, int chunkX, int chunkZ) {
		...
		// material please
		girderMaterial = platmap.generator.materialProvider.itemsSelectMaterial_UnfinishedBuildings
		...
		for (int floor = 0; floor < depth; floor++) {
			...
			if (floor == 0)
				drawFence(generator, chunk, context, 0, generator.streetLevel + 1, floor, neighborBasements,
						me.daddychurchill.CityWorld.Support.MaterialTags.pickSiteFence(platmap.getOddsGenerator()), 3);
		...
		if (!unfinishedBasementOnly) {
			...
			if (needStairsDown) {
				for (int floor = 0; floor < depth; floor++) {
					...
					// plain walls please
					drawStairsWalls(generator, chunk, floorAt, basementFloorHeight, StairWell.CENTER, Material.AIR,
							false, floor == depth - 1);
				...
				for (int floor = 0; floor < height; floor++) {
					...
					if (floor <= floorsBuilt) {
						...
						if (floor > 0 || (floor == 0 && (depth > 0 || height > 1)))
							drawStairsWalls(generator, chunk, floorAt, aboveFloorHeight, StairWell.CENTER, Material.AIR,
