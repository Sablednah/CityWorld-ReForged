...
public class OldCastleLot extends ConstructLot {
	...
	public OldCastleLot(PlatMap platmap, int chunkX, int chunkZ) {
		...
		platformMaterial = platmap.generator.materialProvider.itemsSelectMaterial_Castles.getRandomMaterial(chunkOdds,
				Material.COBBLESTONE);

		if (chunkOdds.playOdds(Odds.oddsPrettyLikely))
			wallMaterial = platformMaterial;
		else
			wallMaterial = platmap.generator.materialProvider.itemsSelectMaterial_Castles.getRandomMaterial(chunkOdds,
					platformMaterial);

		if (chunkOdds.playOdds(Odds.oddsSomewhatLikely))
			supportMaterial = platformMaterial;
		else
			supportMaterial = platmap.generator.materialProvider.itemsSelectMaterial_Castles
					.getRandomMaterial(chunkOdds, platformMaterial);
...
//		int thirdZ1 = chunkRandom.nextBoolean() ? secondZ1 : secondZ1 + 4;
		...
		// always an ex-castle — weathered rubble, never burning (intensity 1.0, fire forced off)
		generator.destroyWithin(originX + insetChaos, originX + 16 - insetChaos, y1, y3,
				originZ + insetChaos, originZ + 16 - insetChaos, 1.0, false);
