		...
		if (stillNeedWalls) {
			...
			if (!heights.toWest()) {
				byteChunk.setBlocks(insetWE, insetWE + 1, y1, y2, insetNS + 1, byteChunk.width - insetNS - 1,
						wallMaterial, glassMaterial, floorWallsNS, BlockFace.NORTH, BlockFace.SOUTH);
				if (outsetEffect)
					byteChunk.setBlocks(insetWE - 1, insetWE, y1, y2 + 1, insetNS + 1, byteChunk.width - insetNS - 1,
							outsetMaterial, outsetBackfill, floorWallsNS, BlockFace.NORTH, BlockFace.SOUTH);
			}
			if (!heights.toEast()) {
				byteChunk.setBlocks(byteChunk.width - insetWE - 1, byteChunk.width - insetWE, y1, y2, insetNS + 1,
						byteChunk.width - insetNS - 1, wallMaterial, glassMaterial, floorWallsNS, BlockFace.NORTH,
						BlockFace.SOUTH);
				if (outsetEffect)
			...
			if (!heights.toNorth()) {
				byteChunk.setBlocks(insetWE + 1, byteChunk.width - insetWE - 1, y1, y2, insetNS, insetNS + 1,
						wallMaterial, glassMaterial, floorWallsWE, BlockFace.EAST, BlockFace.WEST);
				if (outsetEffect)
					byteChunk.setBlocks(insetWE + 1, byteChunk.width - insetWE - 1, y1, y2 + 1, insetNS - 1, insetNS,
							outsetMaterial, outsetBackfill, floorWallsWE, BlockFace.EAST, BlockFace.WEST);
			}
			if (!heights.toSouth()) {
				byteChunk.setBlocks(insetWE + 1, byteChunk.width - insetWE - 1, y1, y2, byteChunk.width - insetNS - 1,
						byteChunk.width - insetNS, wallMaterial, glassMaterial, floorWallsWE, BlockFace.EAST,
						BlockFace.WEST);
				if (outsetEffect)
					byteChunk.setBlocks(insetWE + 1, byteChunk.width - insetWE - 1, y1, y2 + 1,
							...
							floorWallsWE, BlockFace.EAST, BlockFace.WEST);
			}
				...
				if (!heights.toNorthWest()) {
					...
					if (outsetEffect)
						byteChunk.setBlocks(0, insetWE, y1, y2 + 1, insetNS - 1, insetNS, outsetMaterial,
								outsetBackfill, floorWallsWE, BlockFace.EAST, BlockFace.WEST);
				}
				if (!heights.toSouthWest()) {
				...
				if (!heights.toNorthEast()) {
					...
					if (outsetEffect)
						byteChunk.setBlocks(byteChunk.width - insetWE, byteChunk.width, y1, y2 + 1, insetNS - 1,
								insetNS, outsetMaterial, outsetBackfill, floorWallsWE, BlockFace.EAST, BlockFace.WEST);
				}
				if (!heights.toSouthEast()) {
			...
			if (heights.toNorth()) {
				if (!heights.toNorthWest()) {
					byteChunk.setBlocks(insetWE, insetWE + 1, y1, y2, 0, insetNS, wallMaterial, glassMaterial,
							floorWallsNS, BlockFace.NORTH, BlockFace.SOUTH);
					if (outsetEffect)
						byteChunk.setBlocks(insetWE - 1, insetWE, y1, y2 + 1, 0, insetNS, outsetMaterial,
								outsetBackfill, floorWallsNS, BlockFace.NORTH, BlockFace.SOUTH);
				}
				if (!heights.toNorthEast()) {
					...
					if (outsetEffect)
						byteChunk.setBlocks(byteChunk.width - insetWE, byteChunk.width - insetWE + 1, y1, y2 + 1, 0,
								insetNS, outsetMaterial, outsetBackfill, floorWallsNS, BlockFace.NORTH,
								BlockFace.SOUTH);
				}
			}
			if (heights.toSouth()) {
				if (!heights.toSouthWest()) {
					...
					if (outsetEffect)
						byteChunk.setBlocks(insetWE - 1, insetWE, y1, y2 + 1, byteChunk.width - insetNS,
								byteChunk.width, outsetMaterial, outsetBackfill, floorWallsNS, BlockFace.NORTH,
								BlockFace.SOUTH);
				}
				if (!heights.toSouthEast()) {
...
//			}
	...
	private void drawInteriorParts(CityWorldGenerator generator, RealBlocks chunk, DataContext context,
			...
			boolean allowRounded, Material materialWall, Material materialGlass, StairWell stairLocation,
	...
	// outside
	protected void drawExteriorDoors(CityWorldGenerator generator, RealBlocks chunk, DataContext context, int floor,
			int floorAt, int floorHeight, int insetNS, int insetWE, boolean allowRounded, Material materialWall,
...
//				wallMaterial = (byte) Material.LAPIS_BLOCK.getMaterial();
	...
	private void drawInteriorDoors(CityWorldGenerator generator, RealBlocks chunk, DataContext context,
			...
			int insetWE, boolean allowRounded, Material materialWall, Material materialGlass, StairWell stairsLocation,
			Surroundings heights) {
...
//				materialWall = Material.GOLD_BLOCK;
	...
	private void drawExteriorDoors(CityWorldGenerator generator, RealBlocks chunk, DataContext context,
			...
			Material materialWall, Material materialGlass, StairWell stairsLocation, Surroundings heights) {
	...
	private void drawInteriorRoomsGrid(CityWorldGenerator generator, RealBlocks chunk, DataContext context,
		...
		if (drawNarrowInteriors) {
			...
			if (!allowRounded) {
				...
				// Northward
				if (heights.toNorth())
					drawInteriorRoom(generator, chunk, rooms, floor, 3, y1, 1, height, BlockFace.NORTH, materialWall,
							materialGlass); // 1
				if (insetNS < 2)
					drawInteriorRoom(generator, chunk, rooms, floor, 8, y1, 2, height, BlockFace.WEST, materialWall,
							materialGlass); // 2

				// Eastward
				if (heights.toEast())
					drawInteriorRoom(generator, chunk, rooms, floor, 12, y1, 3, height, BlockFace.EAST, materialWall,
							materialGlass); // 3
				if (insetWE < 2)
					drawInteriorRoom(generator, chunk, rooms, floor, 11, y1, 8, height, BlockFace.NORTH, materialWall,
							materialGlass); // 4

				// Southward
				if (heights.toSouth())
					drawInteriorRoom(generator, chunk, rooms, floor, 11, y1, 12, height, BlockFace.SOUTH, materialWall,
							materialGlass); // 5
				if (insetNS < 2)
					drawInteriorRoom(generator, chunk, rooms, floor, 5, y1, 11, height, BlockFace.EAST, materialWall,
							materialGlass); // 6

				// Westward
				if (heights.toWest())
					drawInteriorRoom(generator, chunk, rooms, floor, 1, y1, 10, height, BlockFace.WEST, materialWall,
							materialGlass); // 7
				if (insetWE < 2)
					drawInteriorRoom(generator, chunk, rooms, floor, 2, y1, 5, height, BlockFace.SOUTH, materialWall,
							materialGlass); // 8
			}
			...
			if (heights.toNorthWest()) {
				if (heights.toNorth() && heights.toWest()) {
					// 1 & 2
					drawInteriorRoom(generator, chunk, rooms, floor, 1, y1, 0, height, BlockFace.EAST, materialWall,
							materialGlass);
					drawInteriorRoom(generator, chunk, rooms, floor, 5, y1, 0, height, BlockFace.WEST, materialWall,
							materialGlass);
				}
				...
				if (heights.toNorth()) {
					if (heights.toWest()) {
						...
						// n & m
						drawInteriorRoom(generator, chunk, rooms, floor, 4, y1, 1, height, BlockFace.SOUTH,
								materialWall, materialGlass);
						drawInteriorRoom(generator, chunk, rooms, floor, 4, y1, 5, height, BlockFace.NORTH,
								materialWall, materialGlass);
					}
				} else {
					if (heights.toWest()
						...
						// c & d
						drawInteriorRoom(generator, chunk, rooms, floor, 1, y1, 4, height, BlockFace.EAST, materialWall,
								materialGlass);
...
//								BlockFace.WEST, materialWall, materialGlass);
						...
						// q
						drawInteriorRoom(generator, chunk, rooms, floor, 4, y1, 4, height, BlockFace.EAST, materialWall,
								materialGlass);
					}
				}
			}
			...
			if (heights.toNorthEast()) {
				if (heights.toNorth() && heights.toEast()) {
					// 3 & 4
					drawInteriorRoom(generator, chunk, rooms, floor, 13, y1, 1, height, BlockFace.SOUTH, materialWall,
							materialGlass);
					drawInteriorRoom(generator, chunk, rooms, floor, 13, y1, 5, height, BlockFace.NORTH, materialWall,
							materialGlass);
				}
			} else if (includeOuterRooms) {
				if (heights.toNorth()) {
					if (heights.toEast()) {
						...
						// g & h
						drawInteriorRoom(generator, chunk, rooms, floor, 9, y1, 1, height, BlockFace.SOUTH,
								materialWall, materialGlass);
...
//								BlockFace.NORTH, materialWall, materialGlass);
					...
					if (heights.toEast()
						...
						// a & b
						drawInteriorRoom(generator, chunk, rooms, floor, 8, y1, 4, height, BlockFace.EAST, materialWall,
								materialGlass);
						drawInteriorRoom(generator, chunk, rooms, floor, 12, y1, 4, height, BlockFace.WEST,
								materialWall, materialGlass);
					} else if (!allowRounded && stairsLocation == StairWell.SOUTHWEST) {
						// r
						drawInteriorRoom(generator, chunk, rooms, floor, 9, y1, 4, height, BlockFace.SOUTH,
								materialWall, materialGlass);
					}
				}
			}
			...
			if (heights.toSouthWest()) {
				if (heights.toSouth() && heights.toWest()) {
					// 5 & 6
					drawInteriorRoom(generator, chunk, rooms, floor, 0, y1, 12, height, BlockFace.NORTH, materialWall,
							materialGlass);
					drawInteriorRoom(generator, chunk, rooms, floor, 0, y1, 8, height, BlockFace.SOUTH, materialWall,
							materialGlass);
				}
			} else if (includeOuterRooms) {
				if (heights.toSouth()) {
...
//								BlockFace.SOUTH, materialWall, materialGlass);
						drawInteriorRoom(generator, chunk, rooms, floor, 4, y1, 12, height, BlockFace.NORTH,
								materialWall, materialGlass);
					}
				} else {
					if (heights.toWest()
						...
						// j & i
						drawInteriorRoom(generator, chunk, rooms, floor, 1, y1, 9, height, BlockFace.EAST, materialWall,
								materialGlass);
						drawInteriorRoom(generator, chunk, rooms, floor, 5, y1, 9, height, BlockFace.WEST, materialWall,
								materialGlass);
					} else if (!allowRounded && stairsLocation == StairWell.NORTHEAST) {
						// t
						drawInteriorRoom(generator, chunk, rooms, floor, 4, y1, 9, height, BlockFace.NORTH,
								materialWall, materialGlass);
					}
				}
			}
			...
			if (heights.toSouthEast()) {
				if (heights.toSouth() && heights.toEast()) {
					// 7 & 8
					drawInteriorRoom(generator, chunk, rooms, floor, 12, y1, 13, height, BlockFace.WEST, materialWall,
							materialGlass);
					drawInteriorRoom(generator, chunk, rooms, floor, 8, y1, 13, height, BlockFace.EAST, materialWall,
							materialGlass);
				}
			} else if (includeOuterRooms) {
				if (heights.toSouth()) {
					if (heights.toEast()) {
						...
						// e & f
						drawInteriorRoom(generator, chunk, rooms, floor, 9, y1, 8, height, BlockFace.SOUTH,
								materialWall, materialGlass);
						drawInteriorRoom(generator, chunk, rooms, floor, 9, y1, 12, height, BlockFace.NORTH,
								materialWall, materialGlass);
					}
				} else {
					if (heights.toEast()
...
//								BlockFace.EAST, materialWall, materialGlass);
						drawInteriorRoom(generator, chunk, rooms, floor, 12, y1, 9, height, BlockFace.WEST,
								materialWall, materialGlass);
					} else if (!allowRounded && stairsLocation == StairWell.NORTHWEST) {
						// s
						drawInteriorRoom(generator, chunk, rooms, floor, 9, y1, 9, height, BlockFace.WEST, materialWall,
								materialGlass);
					}
				}
			}
	...
	private void drawInteriorWEWall(RealBlocks chunk, int x, int y1, int y2, int z, Material wallMaterial,
			Material glassMaterial) {
		drawInteriorWEWall(chunk, x, x + 8, y1, y2, z, wallMaterial, glassMaterial);
	}

	private void drawInteriorNSWall(RealBlocks chunk, int x, int y1, int y2, int z1, int z2, Material wallMaterial,
			Material glassMaterial) {
		chunk.setBlocks(x, x + 1, y1, y2, z1, z2, wallMaterial, glassMaterial, wallsInterior, BlockFace.NORTH,
				BlockFace.SOUTH);
	}

	private void drawInteriorWEWall(RealBlocks chunk, int x1, int x2, int y1, int y2, int z, Material wallMaterial,
			Material glassMaterial) {
		chunk.setBlocks(x1, x2, y1, y2, z, z + 1, wallMaterial, glassMaterial, wallsInterior, BlockFace.EAST,
				BlockFace.WEST);
	}
...
//							CornerStyle.FILLED, allowRounded, false, material, material, heights);
	...
	private void drawEdgedRoof(CityWorldGenerator generator, InitialBlocks chunk, DataContext context, int y1,
