...
| `cityworld:fittings/fence` | the garden fence: house railings and park edges | the vanilla wooden fences by id + Macaw's picket fences and hedges |
...
| `cityworld:fittings/site_fence` | the tall fence round a construction site or a factory yard | iron bars + Macaw's wired (barbed-wire) fences |
...
| `cityworld:fittings/stairs` | the staircase of a MODERN house | **empty** — Macaw's compact and terrace stairs when installed (their matching railing, platform and balcony are found by name); the vanilla stair run otherwise |
...
- **Roofs are matched by name.** A MODERN house whose roof is `oak_planks` looks for `oak_planks_roof`
  ...
  roof mod, in stairs. The stepped layers become stairs facing the ridge; corners and ridge caps
  (`*_top_roof`) are shaped by the roof block's own neighbour logic, exactly as if a player had built
  it. CLASSIC keeps its stepped roof.
...
**Left out on purpose:** Macaw's *metal* fences and its two-tall *gates* (own connection logic that
generation-stage placement cannot drive — a gate's top half on its own was measured in the first
sweep, which is why the fence pools list the vanilla fences by id rather than `#minecraft:wooden_fences`,
where Macaw's puts its gates too); garage doors (a different block shape); Macaw's loft, skyline and
bulk stairs (a ramp, and shapes that spill into the next cell — the office stairwells still draw
vanilla stairs); tiki torches and garden lights (no placer yet — campgrounds, beaches and park paths
would want them).
